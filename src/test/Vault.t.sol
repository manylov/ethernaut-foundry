...
contract VaultTest is DSTest {
    ...
    function testVaultHack() public {
        ...
        // uint256 slot = vm.target(address(ethernautVault)).sig(ethernautVault.password.selector).with_key(1).find();

        bytes32 password = vm.load(address(ethernautVault), bytes32(abi.encode(1)));
